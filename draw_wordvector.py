...
data_path = basic_path + '/' + 'corpus/'
save_path = basic_path + '/' + 'pictures/'
doc = "full_text_en"

words_frequency = "words_frequency_no_stopwords.txt"
...
makewordvector(doc, 10)     # 注意第二个参数！中文语段较少，相应地最小计数词频也应小，不然没几个词会出现在图中
...
def tsne_plot(model, chosen_words, picname):
    ...
    plt.legend([dot1], ['Frequently used words'], fontsize = 'xx-large')
    # plt.title('Word Vectors')
    plt.savefig(save_path + '{}.jpg'.format(picname))
    plt.clf()

chosen_words = pickwords(data_path + words_frequency, 400)
print ("chosen_words")

tsne_plot(model, chosen_words, 'chosen_word_vector')
